      ...
      {selection.status === "pending" || selection.status === "lost_upload" ? (
        ...
        <>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            <MetricStat label="Customers" value={vm.summary.totalCustomers.toLocaleString()} />
            <MetricStat
            ...
            />
            <MetricStat
              label="First-to-second within 90 days"
            ...
            />
            <MetricStat label="Avg days first → second" value={formatDays(vm.summary.averageDaysToSecondOrder)} />
            <MetricStat label="Median days first → second" value={formatDays(vm.summary.medianDaysToSecondOrder)} />
            <MetricStat
            ...
            />
            <MetricStat
              label="Avg Month +2 active rate"
              value={formatPct(vm.summary.averageMonthPlus2ActiveRate)}
              metricId="cohort_retention"
            />
            <MetricStat
              label="Avg Month +3 active rate"
              value={formatPct(vm.summary.averageMonthPlus3ActiveRate)}
              metricId="cohort_retention"
            />
          </div>
          ...
          <p className="rounded-lg border border-zinc-200/90 bg-white px-4 py-3.5 text-sm leading-relaxed text-zinc-700 shadow-sm ring-1 ring-black/[0.02]">
            <strong className="font-semibold text-zinc-900">How to read these metrics.</strong> Month +N active rate is
            the share of the cohort who placed at least one order in calendar month acquisition&nbsp;month&nbsp;+&nbsp;N
            (UTC boundaries). First-to-second within 90 days is a separate journey metric: among all customers, the share
            whose second order occurred within 90 days of their first — regardless of calendar month alignment.
          </p>

          <AnalyticalPanel
            title="Retention by cohort"
            description="Each row is customers acquired in that first-order month. Active rates count shoppers with ≥1 qualifying order in the target calendar month (see metric engine definitions). Month +6 appears when the active dataset timeline includes that horizon for a cohort."
          >
