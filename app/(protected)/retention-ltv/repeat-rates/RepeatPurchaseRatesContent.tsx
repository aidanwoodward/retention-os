...
export default function RepeatPurchaseRatesContent() {
  ...
  const fetchRepeatData = useCallback(async () => {
    try {
      ...
      // If API returns non-OK, log a single WARN in dev and fall back to dummy data
      if (!response.ok) {
        if (process.env.NODE_ENV === 'development') {
          console.warn(`[Repeat Purchases] API returned ${response.status} ${response.statusText}, using fallback dummy data`);
        }
        setRepeatData(null);
        setError(null);
        return;
      ...
      let data: RepeatPurchaseResponse;
      try {
        data = await response.json();
      } catch (parseError) {
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Repeat Purchases] Failed to parse JSON response, using fallback dummy data', parseError);
        }
        setRepeatData(null);
        setError(null);
        return;
      ...
      // Validate response shape - check for required fields
      if (!data || typeof data !== 'object') {
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Repeat Purchases] Invalid response shape (not an object), using fallback dummy data');
        }
        setRepeatData(null);
        setError(null);
        return;
      ...
      if (data.success && data.data && Array.isArray(data.data.purchaseBreakdown) && data.data.purchaseBreakdown.length > 0) {
        // Validate required fields exist
        const requiredFields = ['totalCustomers', 'secondPurchaseRate', 'medianPurchases', 'customersWith3PlusPurchases', 'calculated_at'];
        const hasAllFields = requiredFields.every(field => field in data.data);
        ...
        if (hasAllFields) {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.warn('[Repeat Purchases] Missing required fields in response, using fallback dummy data');
          }
        }
      } else {
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Repeat Purchases] Invalid or empty data structure, using fallback dummy data');
        }
      }
      
      // Fall back to dummy data if validation fails
      setRepeatData(null);
      setError(null);
    } catch (err) {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        // In dev, silently fall back to dummy data (already logged warning above)
        if (process.env.NODE_ENV === 'development') {
          console.warn('[Repeat Purchases] Fetch error, using fallback dummy data:', err);
        }
        setRepeatData(null);
        setError(null);
      }
