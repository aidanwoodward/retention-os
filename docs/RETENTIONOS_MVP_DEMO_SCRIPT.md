...
Companion app: **`restart-retentionos-mvp` spine.** The MVP spine contains six analytical routes — `/dashboard`, `/cohorts`, `/retention`, `/ltv`, `/acquisition`, and `/products` — plus `/insights` and `/data`, for eight routes in total.
...
“In the next few minutes we’ll walk RetentionOS’s Revenue Durability Command Centre. The KPI spine runs from **either** the **canonical demo dataset** **or** an **uploaded CSV snapshot you save for this browser session** — same **metric engine** (`/lib/metrics`) and **rules-based insights** (`/lib/insights`) either way, so numbers stay reconcilable across tabs. The Dashboard gives you posture and KPIs at a glance; Cohorts and Retention unpack where quality and timing diverge; LTV ladders separate **net revenue story** from **contribution story**; Acquisition and Products surface CAC / LTV:CAC / payback and first-product customer quality when inputs unlock; Insights surfaces **prioritized moves** using explicit rules — not AI; and **Data** is the **source control centre**: demo vs session CSV, validation preview, and honest limits. Uploads are **session-only** (**not** persisted to Supabase). **Live Shopify does not power this spine.** Channel-quality engine work, scenario modelling beyond stubs, a formal scored 0–100 Revenue Durability Score, and the Signal / Matrix / Provenance shared layer remain deferred.”
...
- **Banner** again: same **Active source** contract as the six analytical routes.
...
- **Route coverage:** The MVP spine contains six analytical routes — Dashboard, Cohorts, Retention, LTV, Acquisition and Products — plus Insights and Data, for eight routes in total; all share the **same browser-tab session selection** as this page.
- **Fixture counts** below stay **demo-aligned** on purpose (audit trail); CSV workflow: validate, preview, **save to sessionStorage**, **Revert to demo dataset** clears upload for the six analytical routes plus Insights (and this Data control surface).
- **Transparency / Not live yet:** Supabase persistence for uploads; production Shopify ingestion; channel-quality engine; numerical 0–100 Revenue Durability Score; scenario modelling beyond stubs; Signal / Matrix / Provenance shared layer; 6B analytical page upgrades. CAC, LTV:CAC, payback, and first-product quality are on the canonical spine (`/acquisition`, `/products`) when spend/line inputs unlock.
...
3. **Show demo source:** Chip **Demo fixture**; **Route coverage** describes the six analytical routes plus Insights using `getDemoDataset()` → `/lib/metrics`.
...
12. **Revert** (clear uploaded dataset): The six analytical routes plus Insights fall back to **Demo dataset** on next navigation; banners align.

### Uploaded-data source-switch QA checklist (code-audited)

| Step | Expectation |
|------|-------------|
| No valid upload in `sessionStorage` | `resolveCommandCentreDatasetSource()` uses `buildDemoRetentionOSDataset()`; the six analytical routes plus Insights show **Demo dataset** in `MetricSourceBanner`. |
...
| `/dashboard` … `/products`, `/insights` | Each analytical/Insights client page calls `resolveCommandCentreDatasetSource()` in `useLayoutEffect` and builds view models from `selection.dataset`; banner **Uploaded CSV session dataset**. |
...
| `clearUploadedRetentionOSDataset()` | Removes session key; revisiting the six analytical routes plus Insights resolves demo again. |
...
- The MVP spine contains six analytical routes — `/dashboard`, `/cohorts`, `/retention`, `/ltv`, `/acquisition`, and `/products` — plus `/insights` and `/data`, for eight routes in total. With **no session upload**, the six analytical routes consume **`getDemoDataset()`** through **`/lib/metrics`**, and `/insights` uses the same source via **`/lib/insights`**. With a **saved session upload**, those routes consume the **parsed `RetentionOSDataset`** from **sessionStorage** for **this tab only**. `/data` is the source-control surface for that selection.
- **Uploaded CSV is not persisted to Supabase** (or otherwise server-synced) in this product path.
- **Live Shopify ingestion does not power** the eight-route command-centre spine; warehouse materialisations and Supabase KPI paths are **inactive** on these surfaces.
...
- **Demo routing guard:** Requests outside the eight-route MVP spine (six analytical routes plus `/insights` and `/data`; except **Settings** and auth flows) **redirect to `/dashboard`**, legacy analytics URLs map back onto the command centre, and **dummy-data POST APIs return 403 in production** so demos do not surface destructive dev tools.
...
- **Selected-source analytical + Insights pages:** `/dashboard`, `/cohorts`, `/retention`, `/ltv`, `/acquisition`, `/products`, and `/insights` resolve **`resolveCommandCentreDatasetSource()`** client-side and feed **`build*ViewModelFromDataset`**; **`/data`** is the eighth spine route — the **source control centre** (hero + route coverage + revert).
- **CAC, LTV:CAC, and payback** on `/acquisition` when aggregate marketing spend unlocks (otherwise Locked).
- **First-product customer quality** on `/products` (three-state attribution; single_product rows).
...
**Not real yet (deferred or unimplemented)**

- **Supabase (or server) persistence** for uploaded datasets and multi-tenant truth.
- **Production Shopify ingestion** powering the command-centre spine.
- **Channel-quality engine** (spend channel labels are not a quality module).
- **Scenario modelling** beyond current quarantined stubs.
- **Numerical 0–100 Revenue Durability Score** — posture remains a **heuristic label** (`Healthy` / `Mixed` / `Watch`), not that composite.
- **Signal / Matrix / Provenance** shared layer and **6B analytical page upgrades**.
...
“Yes on `/acquisition` when you enter aggregate marketing spend on **Data** — CAC, LTV:CAC, and payback unlock from the same engine. Without spend, those tiles stay Locked rather than inventing zeros. Channel-quality scoring is still deferred.”
...
1. **Tenant persistence & connectors** — server-side storage, isolation, and production Shopify (or warehouse) ingestion **without** changing the honest demo/CSV spine narrative.
2. **Signal / Matrix / Provenance + 6B page upgrades** — shared analytical chrome and richer presentation of already-shipped metrics.
3. **Channel quality** — bridge creative and channel payloads to dispersion patterns responsibly (engine not shipped).
4. **Scenario modelling** — stress ladders and posture assumptions under spend and elasticity scenarios.
5. **Numerical Revenue Durability Score methodology** — optional future 0–100 composite with published inputs/weights (today posture is heuristic label only).
