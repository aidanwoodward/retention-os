...
export default function CustomerSegmentsPage() {
  ...
  const getSegmentPerformance = (segment: CustomerSegmentData) => {
    if (segment.retention_rate > 80) return { level: 'excellent', color: 'text-green-600', bg: 'bg-green-50' };
    if (segment.retention_rate > 50) return { level: 'good', color: 'text-blue-600', bg: 'bg-blue-50' };
  ...
  const getSegmentIcon = (segmentType: string) => {
    ...
    return icons[segmentType] || Activity;
  };
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-red-50 border border-red-200 rounded-2xl p-8 text-center">
          ...
          <h3 className="text-lg font-semibold text-red-800 mb-2">Error Loading Customer Segments</h3>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 rounded-2xl p-8 text-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-violet-100 text-lg">Advanced customer segmentation with behavioral and value-based analytics</p>
            </div>
            <div className="text-right">
              <div className="text-3xl font-bold">{segments.length}</div>
              <div className="text-violet-100">Active Segments</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-gradient-to-br from-violet-50 to-violet-100 rounded-2xl p-6 border border-violet-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-violet-600 text-sm font-medium">Total Segments</p>
              <p className="text-3xl font-bold text-violet-900">{segments.length}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-900">
                {formatCurrency(segments.reduce((sum, s) => sum + s.total_revenue, 0))}
              </p>
            </div>
        ...
        <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-6 border border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-blue-600 text-sm font-medium">Best Segment</p>
              <p className="text-3xl font-bold text-blue-900">
                {segments.length > 0 ? segments.reduce((best, s) => s.retention_rate > best.retention_rate ? s : best).segment_name.slice(0, 10) + '...' : 'N/A'}
              </p>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-2xl p-6 border border-purple-200">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-600 text-sm font-medium">Avg Retention</p>
              <p className="text-3xl font-bold text-purple-900">
                {segments.length > 0 ? (segments.reduce((sum, s) => sum + s.retention_rate, 0) / segments.length).toFixed(1) : 0}%
              </p>
            </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {segments.map((segment) => {
                  const performance = getSegmentPerformance(segment);
                  const Icon = getSegmentIcon(segment.segment_type);
                  return (
                    <tr key={segment.segment_name} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <Icon className="w-5 h-5 text-gray-400 mr-3" />
                          <span className="text-sm font-medium text-gray-900">{segment.segment_name}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        {segment.retention_rate.toFixed(1)}%
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
