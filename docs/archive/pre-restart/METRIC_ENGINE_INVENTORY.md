...
> **Historical audit — superseded for current architecture.** This Sprint A–E inventory is retained as evidence of the state inspected in May 2026. Use [`RETENTIONOS_ARCHITECTURE.md`](../../RETENTIONOS_ARCHITECTURE.md) for the current canonical path, eight-route spine, complete route/API inventory, and dispositions. Do not use classifications or counts below without reproducing them against the current commit.
...
**Related docs:** `docs/archive/pre-restart/active-metrics-map.md`, `docs/archive/pre-restart/metric-inventory-audit.md` (page-level audits; some routes/components have moved since — this doc reflects the repo as inspected).
...
| **Revenue cohorts** | `app/(protected)/retention-ltv/revenue-cohorts/page.tsx` | Trend series, top-cohort shares, period comparisons (see `docs/archive/pre-restart/active-metrics-map.md`). |
