...
> **Supporting historical milestone.** This document remains the detailed record of session CSV ingestion at the checkpoint date. Its six-route references and capability-gap lists (including CAC, payback, and product quality as gaps) are **frozen historical claims** and are not current product truth — those capabilities later joined the retained spine. Current capability and roadmap: [`RETENTIONOS_ARCHITECTURE.md`](../../RETENTIONOS_ARCHITECTURE.md), [`PRODUCT_RECONCILIATION_BACKLOG.md`](../../PRODUCT_RECONCILIATION_BACKLOG.md). Current metric semantics: [`METRIC_CONTRACTS.md`](../../METRIC_CONTRACTS.md). Do not rewrite the historical body below.
